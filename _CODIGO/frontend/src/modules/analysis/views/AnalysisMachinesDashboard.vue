...
/**
 * Vista principal del módulo de Análisis.
 * Integra la visualización y edición 2D/JSON/Formulario del modelo CIM
 * y de las máquinas individuales que lo componen.
 */
...
/**
 * Calcula el ID del proyecto actual basado en la ruta.
 * @returns {number} ID del proyecto.
 */
const projectId = computed(() => Number(route.params.id))

/**
 * Obtiene la máquina actualmente seleccionada desde el store.
 * @returns {CimMachine | null} Máquina actual o null si no hay selección.
 */
...
/**
 * Genera la lista de migas de pan (breadcrumbs) para la navegación jerárquica.
 * @returns {Array} Lista de elementos breadcrumb.
 */
...
/**
 * Maneja la exportación del estado actual (CIM o Máquina individual) a formato JSON.
 */
const handleExport = () => {
...
/**
 * Dispara el evento de edición básica dependiendo del contexto seleccionado.
 */
...
/**
 * Confirma y ejecuta el borrado de un componente (subnodo) o una máquina completa.
 */
...
/**
 * Calcula los detalles de advertencia para mostrar al intentar eliminar un elemento.
 * Informa sobre dependencias que también se verán afectadas.
...
/**
 * Establece el modo de visualización principal.
 * @param {'2D' | 'JSON' | 'FORM'} mode El modo a activar.
 */
...
/**
 * Eventos emitidos por el dashboard.
 */
...
/**
 * Abre el modal para añadir una nueva máquina CIM si no hay cambios sin guardar.
 */
...
/**
 * Confirma la creación de una nueva máquina CIM.
 * @param {string} name Nombre de la máquina.
 * @param {string} description Descripción de la máquina.
 */
...
/**
 * Selecciona la creación de un nuevo subnodo (elemento interno) dentro de la máquina actual.
 * @param {'el'} type Tipo del subnodo.
 */
...
<template>
  <div class="analysis-dashboard flex flex-col h-full bg-geist-bg">
    ...
    <div v-else class="w-full h-full flex items-center justify-center">
      <div v-if="store.loading" class="flex items-center justify-center h-full w-full">
        <BaseLoader />
      </div>
