...
Para la ejecución local del proyecto MDAA, necesitaremos tener instalado docker compose. Como se podrá ver en el \textit{README.md} antes mencionado, tendremos una serie de scripts que facilitarán tanto la ejecución de la herramienta como la comprobación de los DSLs desarrollados, así como utilizades como el incluir los casos de uso en el entorno local instalado. Para ejecutarlos, se necesita una terminal linux-like (como puede ser la terminal de Linux, WSL de Windows, o la terminal de MacOS). Para la ejecución, compilación y validación de los DSL, como del script \texttt{validar-test.sh} (también comentado en el \textit{README.md} antes mencionado), se necesita tener instalado nodejs. Por último, para el script de instalación de lso casos de prueba en la aplicación local, necesitaremos tener instalado tanto python 3.x como el conector mysql para python (\texttt{pip3 install mysql-connector-python}).
